...
(async function(){
    ...
    const wasm = fs.readFileSync(path.join(__dirname, "../orgwasm/lib/typedarray.wasm"))
    let { module: wasmModule, instance: wasmInstance } = await WebAssembly.instantiate(wasm, imports);
    function wasmTest() {
        ...
        for (let i = 0; i < count; i++) {
            let result = wasmInstance.exports.addDotArray(
                size,
                addMe.x, addMe.y, addMe.z,
                dotMe.x, dotMe.y, dotMe.z
            )
            // console.log(result)
        }
        let total = 0
    ...
    function run(...tests) {
        for (let test of tests) {
            ...
            let time = times.slice(-lastSamplesToAverage).reduce((a, b) => a + b) / lastSamplesToAverage
            console.log(chalk.green(test.name + ": " + Math.round(time) + " ms"))
        }
    }
    run(
        wasmTest,
